...
class PlexEventHandler(object):
    ...
    def get_activity(self):
        response = requests.get("https://tautulli.papini.dantebarba.com.ar/api/v2?apikey={api_key}&cmd=get_activity".format(api_key=os.getenv("TAUTULLI_API_KEY")))
        self.parse_sessions(response.json()["response"]["data"]["sessions"])

    def get_metadata(self, id):
        response = requests.get("https://tautulli.papini.dantebarba.com.ar/api/v2?apikey={api_key}&cmd=get_metadata&rating_key={id}".format(api_key=os.getenv("TAUTULLI_API_KEY"), id=id))
